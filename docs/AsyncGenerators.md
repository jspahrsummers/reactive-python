# Asynchronous generator semantics and types
...
Here, I attempt to more clearly capture the _actual_ behaviors, based on what I've learned and observed. See also [PEP 492 -- Coroutines with async and await syntax](https://www.python.org/dev/peps/pep-0492/) and [PEP 525 -- Asynchronous Generators](https://www.python.org/dev/peps/pep-0525/)
...
class AsyncGenerator(ABC, AsyncIterator[TYield], Generic[TYield, TSend]):
    ...
    The lifetime of an AsyncGenerator looks as follows:
    ...
    through an exception), the asynchronous generator-iterator instance may
    not be restarted. However, a new one can be obtained by calling the
    function again:
    ...
    async def __anext__(self) -> TYield:  # throws: StopAsyncIteration, ...
        ...
        attempts to _explicitly_ raise StopIteration in its implementation,
        it will instead be converted into a RuntimeError, per PEP 479.)
    ...
    async def athrow(
    ...
    ) -> TYield:  # throws: exc_type, StopAsyncIteration, ...
        """
        ...
        After raising the exception inside the generator, athrow() behaves
        exactly like __anext__().
        
        If the generator has already exited (gracefully or through an
        exception) or been closed previously, nothing happens, and the
        awaitable returned by athrow() will raise a StopAsyncIteration
        exception.
        ...
        _explicitly_ raise StopIteration in its implementation, it will
        instead be converted into a RuntimeError, per PEP 479.)
